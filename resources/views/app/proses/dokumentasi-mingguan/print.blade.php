...
<body>
    <div class="text-center">
        ...
        <h5>DOKUMENTASI PROYEK MINGGU {{ $data->minggu_ke }} ({{ $range }})</h5>
    </div>
    ...
    <div>
        @foreach($listGambar as $gambar)
            <div class="image-block">
                @if (strpos($gambar['file'], 'http') !== false)
                    <img src="{{ $gambar['file'] }}" alt="Gambar">
                @else
                    <img src="{{ asset('storage/' . $gambar['file']) }}" alt="Gambar">
                @endif
                <p>{{ strToUpper($gambar['keterangan']) }}</p>
            </div>
        @endforeach
    </div>
