    ...
    <div class="container mt-4">
        ...
        <table class="table table-bordered table-hover mt-3" style="width: 100%; font-size: 12px; line-height: 0; border-collapse: collapse;">
            ...
            @foreach($dataPekerjaanById as $idPekerjaan => $data)
                ...
                @endif
            @endforeach
            @foreach ($dataCco as $cco)
                @php
                    $totalHargaCco += ($cco->harga * $cco->volume);
                @endphp
                <tr>
                    <td></td>
                    <td colspan="3"><b>{{ $cco->nama }}</b></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td class="text-right"><b>{{ $cco->volume }}</b></td>
                    <td class="text-right"><b>{{ number_format($cco->harga * $cco->volume, 2) }}</b></td>
                </tr>
            ...
            @endphp
            <tr>
                <td colspan="4" class="text-right"><b>TOTAL PEKERJAAN ({{ toRoman($firstLoop) }} - {{ toRoman($lastLoop) }}) + CCO</b></td>
                <td colspan="3" class="text-right"><b>{{ number_format($totalHargaModalFinal, 2) }}</b></td>
                <td colspan="2" class="text-right"><b>{{ number_format($totalHargaJualFinal + $totalHargaCco, 2) }}</b></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td colspan="2" class="text-right"><b>{{ number_format(($totalHargaJualFinal + $totalHargaCco) * 11 / 100, 2) }}</b></td>
            </tr>
            <tr>
                ...
                <td colspan="2" class="text-right"><b>{{ number_format(($totalHargaJualFinal + $totalHargaCco) * 11 / 100 + $totalHargaJualFinal + $totalHargaCco, 2) }}</b></td>
            </tr>
        </table>
    </div>
